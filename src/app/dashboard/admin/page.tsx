...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    ...
    try { const res = await fetch("/api/admin/dashboard"); if (res.ok) setD(await res.json()); } catch {}
    setLoading(false);
  };
  useEffect(() => { loadData(); }, []);
  ...
  return (
    <div className="bg-[#f8f9ff] h-screen flex overflow-hidden">
      <aside className={`hidden md:flex flex-col h-full p-4 gap-2 bg-[#eff4ff] border-r border-[#c7c4d8] ${sidebar ? "w-64" : "w-16"} transition-all duration-200`}>
        ...
        <div className="flex items-center gap-3 px-2 py-3 bg-gray-200/50 rounded-lg mb-2"><div className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-sm font-bold text-gray-600">AD</div><div className={sidebar ? "" : "hidden"}><p className="text-sm font-bold text-gray-900">Admin User</p><p className="text-xs text-gray-400">Super Admin</p></div></div>
        <nav className="flex-1 flex flex-col gap-1">
          {tabs.map((t) => {
            const I = t.icon;
            return (<button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium transition-all ${tab === t.id ? "bg-indigo-100 text-indigo-600 font-bold scale-95 active:scale-90" : "text-gray-500 hover:bg-gray-200/50"}`}>
              <I className="h-5 w-5 flex-shrink-0" />{sidebar && <span>{t.label}</span>}
            </button>);
          })}
        </nav>
        <div className="mt-auto pt-4 border-t border-gray-200 flex flex-col gap-2">
          <button className="w-full py-2.5 bg-indigo-600 text-white rounded-lg font-bold shadow-md hover:bg-indigo-700 transition-all text-sm flex items-center justify-center gap-2"><span>⚡</span> Go Online</button>
          <button className="flex items-center gap-3 px-3 py-2 text-xs text-gray-500 hover:bg-gray-200/50 rounded-lg"><HelpCircle className="h-4 w-4" /> Help Center</button>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto relative">
        <header className="bg-white shadow-sm sticky top-0 z-40 h-16 flex justify-between items-center px-6 border-b border-gray-100">
          <div className="flex items-center gap-4"><button onClick={() => setSidebar(!sidebar)} className="md:hidden p-2 text-gray-400"><Menu className="h-5 w-5" /></button><div className="hidden md:flex items-center gap-3 bg-gray-50 px-4 py-1.5 rounded-full border border-gray-100"><Search className="h-4 w-4 text-gray-400" /><input className="bg-transparent border-none text-sm w-64 outline-none" placeholder="Search audit logs..." /></div></div>
          <div className="flex items-center gap-4"><span className="text-indigo-600 font-bold text-sm border-b-2 border-indigo-600 pb-1">Find Tutors</span><span className="text-gray-400 text-sm hover:text-indigo-600">Courses</span><span className="text-gray-400 text-sm hover:text-indigo-600">Become a Tutor</span></div>
        </header>
          ...
          {tab === "overview" && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { label: "Total Active Tutors", value: (d.tutors?.length || 0).toLocaleString(), icon: Users, iconColor: "bg-indigo-100 text-indigo-600", trend: "+12%", accent: "hover:border-indigo-400" },
                  { label: "Pending Applications", value: (d.pendingTutors?.length || 0).toString(), icon: FileText, iconColor: "bg-amber-100 text-amber-700", badge: "Priority", badgeColor: "bg-amber-100 text-amber-700", accent: "hover:border-amber-400" },
                  { label: "Platform Revenue", value: `$${(d.totalRevenue || 0).toLocaleString()}`, icon: DollarSign, iconColor: "bg-emerald-100 text-emerald-600", sub: "Last 30 Days", accent: "hover:border-emerald-400" },
                ].map((s) => {
                  const I = s.icon;
                  return (<div key={s.label} className="bg-white/70 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-gray-100 transition-all duration-300 group hover:shadow-md" style={{background:"rgba(255,255,255,0.7)",backdropFilter:"blur(12px)",border:"1px solid rgba(226,232,240,0.8)"}}>
                    <div className="flex justify-between items-start mb-4"><div className={`p-3 ${s.iconColor} rounded-lg`}><I className="h-6 w-6" /></div>
                      {s.trend && <span className="text-emerald-600 font-bold flex items-center text-sm">+12% <span className="ml-1">↑</span></span>}
                      {s.badge && <span className={`px-2 py-1 rounded-full text-xs font-bold ${s.badgeColor}`}>Priority</span>}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <section className="lg:col-span-8 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left"><thead className="bg-gray-100"><tr>{["Tutor Name", "Subject", "Date Applied", "Actions"].map(h => (<th key={h} className="px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{h}</th>))}</tr></thead>
                      <tbody className="divide-y divide-gray-50">
                        {(d.pendingTutors || []).length > 0 ? (d.pendingTutors as any[]).slice(0,5).map((t: any, i: number) => (
                          <tr key={t.id||i} className="hover:bg-gray-50 transition-colors">
                            <td className="px-5 py-4"><div className="flex items-center gap-3"><img src={`/images/stitch/admin_management-${i%3}.jpg`} alt="" className="w-10 h-10 rounded-full object-cover border-2 border-white" /><div><p className="font-semibold text-gray-900 text-sm">{t.full_name||"Unknown"}</p><p className="text-xs text-gray-400">{t.email}</p></div></div></td>
                            <td className="px-5 py-4"><span className={`px-3 py-1 rounded-md text-xs font-medium ${i === 0 ? "bg-indigo-100 text-indigo-600" : i === 1 ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-700"}`}>{t.subjects_taught?.[0]||"Pending"}</span></td>
                            <td className="px-5 py-4 text-sm text-gray-400">{t.created_at ? new Date(t.created_at).toLocaleDateString() : "-"}</td>
                            <td className="px-5 py-4"><div className="flex gap-2"><button className="p-1 text-emerald-500 hover:bg-emerald-50 rounded-md transition-colors" title="Approve"><span className="text-lg">✓</span></button><button className="p-1 text-red-500 hover:bg-red-50 rounded-md transition-colors" title="Reject"><span className="text-lg">✕</span></button></div></td>
                          </tr>
                ...
                <section className="lg:col-span-4 bg-gray-100 rounded-xl shadow-sm flex flex-col h-[500px] border border-gray-100">
                  ...
                  <div className="p-5 space-y-4 overflow-y-auto flex-1">
                    {(d.logs||[]).slice(0,6).map((l: any, i: number) => (
                      <div key={l.id||i} className={`flex gap-3 border-l-2 pl-4 py-1 ${i % 3 === 0 ? "border-indigo-600" : i % 3 === 1 ? "border-emerald-500" : "border-red-500"}`}>
                        <div><p className="text-sm font-bold text-gray-900">{l.action?.replace(/_/g," ")||"Event"}</p><p className="text-xs text-gray-400">{l.details ? JSON.stringify(l.details).slice(0,50) : "System activity"}</p><span className="text-[10px] text-gray-400 uppercase tracking-wider">{l.created_at ? new Date(l.created_at).toLocaleString() : ""}</span></div>
                      </div>
                    ))}
                    {(!d.logs||d.logs.length===0) && <p className="text-center text-gray-400 py-8 text-sm">No audit logs</p>}
                  </div>
                </section>
              </div>

              <section className="bg-white/70 backdrop-blur-sm p-8 rounded-2xl relative overflow-hidden h-64 flex items-center justify-center border border-gray-100" style={{background:"rgba(255,255,255,0.7)",backdropFilter:"blur(12px)"}}>
                ...
                <div className="relative z-10 text-center"><h5 className="text-xl font-bold text-gray-900 mb-2">Revenue Insights Engine</h5><p className="text-gray-500 mb-6">Machine learning analysis of platform growth and seasonal trends.</p><button className="bg-indigo-600 text-white px-8 py-3 rounded-full font-bold flex items-center gap-3 mx-auto hover:scale-105 transition-transform">Generate Detailed Forecast <span>✨</span></button></div>
              </section>
            </>
          )}
...
function Applications({d}:any){const apps=(d.applications||[]).filter((a:any)=>a.status!=="draft"); return <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">{apps.length===0?<div className="p-12 text-center"><FileText className="h-10 w-10 text-gray-300 mx-auto mb-3"/><p className="text-gray-400 text-sm">No applications</p></div>:<div className="divide-y divide-gray-50">{apps.map((a:any)=>(<div key={a.id} className="p-5 flex items-center justify-between hover:bg-gray-50"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-sm font-bold text-indigo-600">{(a.full_name||"?").charAt(0)}</div><div><p className="font-semibold text-gray-900 text-sm">{a.full_name||"Unknown"}</p><p className="text-xs text-gray-400">{a.email}</p></div></div><a href={`/dashboard/admin/applications/${a.id}`} className="text-xs text-indigo-600 hover:underline font-semibold">Review →</a></div>))}</div>}</div>}
